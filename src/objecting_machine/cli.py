...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Objecting-Machine thinks for you about the meanings.",
        epilog="Example:  objecting-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | objecting-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | objecting-machine         #

    $ objecting-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="objecting-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"objecting-machine: {utterance}.")
        syslog.closelog()
